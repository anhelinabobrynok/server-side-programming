...
class CinemaReportAPI(APIView):
    ...
    def get(self, request):
        try:
            movies_qs = Movie.objects.all().values('release_year', 'rating')
            ...
            if not df.empty:
                ...
                yearly_stats = df.groupby('release_year')['rating'].mean().reset_index()

                yearly_stats['rating'] = yearly_stats['rating'].round(2)
                yearly_stats = yearly_stats.sort_values('release_year')

                yearly_stats = yearly_stats.rename(columns={'rating': 'popularity'})

                popularity_stats = yearly_stats.to_dict('records')

            total_movies = Movie.objects.count()
            aggregation = Movie.objects.aggregate(avg_val=Avg('duration'))
            avg_duration = aggregation['avg_val'] if aggregation['avg_val'] else 0

            report_data = {
                "report_name": "Cinema General Statistics",
                "total_movies_count": total_movies,
                "average_duration_minutes": round(avg_duration, 1),
                "popularity_by_year": popularity_stats, 
                "status": "success"
            }
